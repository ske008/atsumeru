...
export default function ParticipantPage() {
  ...
  const submittedAmount = useMemo(() => {
    if (!justSubmitted) return 0;
    const row = responses.find((r) => r.id === justSubmitted.responseId);
    return row?.amount ?? defaultDisplayAmount;
  }, [justSubmitted, responses, defaultDisplayAmount]);
      ...
      <div className="stack">
        ...
        {justSubmitted && (
          <div className="card">
            ...
            {submittedAmount > 0 && (
              <p className="hint" style={{ marginTop: 4 }}>&yen;{submittedAmount.toLocaleString()}</p>
            )}
            {event.pay_url ? (
              <>
                <div style={{ marginTop: 12 }}>
